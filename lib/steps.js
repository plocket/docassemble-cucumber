...
// Add a check for an error page before each step
...
When(/I am using a (.*)/, async ( device ) => {
...
Then(/I should see the var(?:iable)? "([^"]+)"(?: with the value "([^"]+)")?/i, async ( var_name, value ) => {  // √
  /* This is theoretical at the moment, not tested.
  * Looks for a field with this variable.
  * Just `buttons` gets you "_multiple_choice" I think, with values of 0, 1, 2, etc.
  * Not yet sure how to add the value into the mix considering it's not always
  * the value - sometimes its the option or part of `name` or something. */
  let base64_name = await scope.base64( scope, var_name );
  let base64_value = await scope.base64( scope, value );
  let base64_first_id = base64_name + '_0';
  let name_attr_B = await scope.base64( scope, `${ base64_name }[B'${ base64_value }']`);
  let name_attr_R = await scope.base64( scope, `${ base64_name }[R'${ base64_value }']`);

  // Schroedinger's cat...
  let [cat_1] = await scope.page.$x(`//*[@id="${base64_first_id}"]`);
  let [cat_3] = await scope.page.$x(`//*[@data-saveas="${base64_name}"]`);
  let [cat_2] = await scope.page.$x(`//*[@name="${base64_name}"]`);
  let [cat_4] = await scope.page.$x(`//*[@name="${name_attr_B}"]`);
  let [cat_5] = await scope.page.$x(`//*[@name="${name_attr_R}"]`);

  // Make the existence of one of them into `true` and the existence of none into `false`
  let variable_is_present = !!(cat_1 || cat_3 || cat_2 || cat_4 || cat_5);
  if ( value ) {
    expect(variable_is_present, `Cannot find the variable "${ var_name }" with the value "${ value }" on the page`).to.be.true;
  } else {
    expect(variable_is_present, `Cannot find the variable "${ var_name }" on the page`).to.be.true;
  }
